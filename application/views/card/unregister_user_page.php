    ...
    <div class="wrapper content">
        <form method="POST" action="" id="register-form">
            ...
            <div class="upload-blk clear">
                <div class="left">
                    <h3 class="fileupload-h3">Upload your Emirates ID</h3>
                    <span class="out_span">(Upload the document here in <strong>pdf format</strong>.)</span>
                </div>

                <div class="upload clear">
                    <span>upload</span>
                    <input type="file" class="input-file" name="id_file" id="id_file" <?= ($new_data && $new_data->emirates_id_file) ? '' : 'required'; ?> accept="application/pdf,.pdf, image/jpg, image/png, image/gif, image/jpeg">
                    <br><br>
                </div>
                <p class="emirates_id_uploaded_file"><?= ($new_data) ? '<a href="' . base_url() . 'uploads/temp_ku_id' . $new_data->emirates_id_file . '">' . $new_data->emirates_id_file . '</a>' : ''; ?></p>
            </div>

            <div class="field single">
                <label for="ku_id">KU ID : <span>(Format: 1000123456)</span></label>
                <input type="text" class="ku_id" name="ku_id" id="ku_id" pattern="^[0-9]+$" required autocomplete="off" value="<?= ($new_data) ? substr($new_data->ku_id, 0, 2) . '*****' . substr($new_data->ku_id, -2) : ($user_data ? substr($user_data->ku_id, 0, 2) . '*****' . substr($user_data->ku_id, -2) : ''); ?>">
            </div>

            <div class="field single">
                <label for="e_id">Emirates ID : <span>(Format: 784000123456789)</span></label>
                <input type="text" class="e_id" name="emirates_id" id="e_id" pattern="^[0-9]+$" required autocomplete="off" value="<?= ($new_data) ? substr($new_data->emirates_id, 0, 3) . '********' . substr($new_data->emirates_id, -4) : ($user_data ? $user_data->emirates_id : ''); ?>">
            </div>
            ...
            <div class="field single">
                ...
                <input type="text" class="country" name="country" id="country" required autocomplete="off" value="<?= ($new_data) ? $new_data->country : ''; ?>">
            </div>

            <div class="field single">
                <label for="graduation">Nationality :</label>
                <input type="text" class="nationality" name="nationality" id="nationality" required autocomplete="off" value="<?= ($new_data) ? $new_data->nationality : ''; ?>">
            </div>
...
<script>
    $(document).ready(function() {
        ...
        $("#country").on("change", function() {
            ...
            if (countryData['iso2'] == 'ae') {
                ...
                <?php if (!($new_data && $new_data->emirates_id_file)) { ?>
    ...
    $("#id_file").on("change", function() {
        ...
        file_name = $('#id_file').val().split('\\').pop();
        $('.emirates_id_uploaded_file').text(file_name);
        ...
        if (!this.checkValidity()) {
            if (this.id == 'ku_id') {
                $.dialog({
                    type: 'orange',
                    title: 'Warning',
                    content: '<strong>The format for entering KU ID:</strong>: 100123456<br>' +
            ...
            if (this.id == 'e_id') {
                $.dialog({
                    type: 'orange',
                    title: 'Warning',
                    content: '<strong>The format for entering:</strong>Emirates ID: 784000123456789<br>' +
                        '(Without special characters)',
                    boxWidth: '30%',
                    useBootstrap: false,
                });
    ...
    $("#register-form").submit(function(e) {
        ...
        var kuid = $('#ku_id').val().substring(0, 4);
        if (kuid != 1000) {
            $.dialog({
                type: 'error',
                title: 'Failed',
                content: 'Invalid KU ID format',
                boxWidth: '30%',
                useBootstrap: false,
            });
            return false;
        ...
        $.ajax({
            url: "<?= base_url('unregister-details') ?>",
            type: "POST",
            dataType: "json",
            data: formData,
            async: false,
